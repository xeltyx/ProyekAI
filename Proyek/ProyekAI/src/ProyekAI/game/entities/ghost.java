...
public class ghost extends character{
    ...
    public ghost(handler handler, float x, float y) {
        ...
        super(handler,x, y,character.def_width*2,character.def_height*2);
        unhurtable=false;
        ...
        anidown = new animation(200,assets.ghost_down);
        aniup = new animation(200,assets.ghost_up);
        anileft = new animation(200,assets.ghost_left);
        aniright = new animation(200,assets.ghost_right);        
    ...
    private BufferedImage getCurrentAnimation(){
        ...
        {
            if(up)
               return assets.ghost_dieup[dieindex];
            else if(down)
                return assets.ghost_diedown[dieindex];
            else if(right)
                return assets.ghost_dieright[dieindex];
            else if(left)
                return assets.ghost_dieleft[dieindex];
        }
        else if(attanimation)
        {
            if(up)
            {
                return assets.ghost_attup[index];
            }
            else if(left)
            {
                return assets.ghost_attleft[index];
            }
            else if(right)
            {
                return assets.ghost_attright[index];
            }
            else if(down)
            {
                return assets.ghost_attdown[index];
            }
        }
        else if(moveup)
        {
